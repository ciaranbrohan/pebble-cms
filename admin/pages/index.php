...
function getPagesFromDirectory($baseDir) {
    ...
    foreach ($dirs as $dir) {
        ...
        foreach ($iterator as $file) {
            // Only process default.md files and skip module directories
            if ($file->isFile() && $file->getFilename() === 'default.md') {
                $relativePath = str_replace($path . '/', '', $file->getPathname());
                $content = file_get_contents($file->getPathname());
                
                // Skip if the parent directory starts with underscore (module)
                $parentDir = basename(dirname($file->getPathname()));
                if (strpos($parentDir, '_') === 0) {
                    continue;
                }
                ...
                if (preg_match('/^---\s*\n(.*?)\n---\s*\n(.*)/s', $content, $matches)) {
                    ...
                    $pages[] = [
                        'title' => $frontmatter['title'] ?? basename(dirname($file->getPathname())),
                        'path' => str_replace($baseDir . '/', '', $file->getPathname()),
                        'template' => $frontmatter['template'] ?? 'default',
                        'group' => $dir,
                        'slug' => $dirPath,
                        'directory_structure' => $dirParts,
    ...
    // Sort pages by their order and directory structure
    usort($pages, function($a, $b) {
        // First compare by directory structure
        $aPath = implode('/', $a['directory_structure']);
        $bPath = implode('/', $b['directory_structure']);
        $pathCompare = strcmp($aPath, $bPath);
        
        if ($pathCompare !== 0) {
            return $pathCompare;
        }
        
        // Then compare by order
...
<div class="bg-white shadow rounded-lg p-6">
    ...
    <div id="<?php echo $dir; ?>-section" class="space-y-6 <?php echo $index === 0 ? '' : 'hidden'; ?>">
        <div class="flex justify-between items-center">
            ...
            <a href="create_page.php?directory=<?php echo urlencode($dir); ?>" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
            ...
            <ul class="divide-y divide-gray-200">
                ...
                <?php else: 
                    ...
                    foreach ($dirPages as $page): 
                        // Check if we need to show a directory header
                        $pagePath = implode('/', $page['directory_structure']);
                        if ($pagePath !== $currentPath && !empty($pagePath)):
                            $currentPath = $pagePath;
                ?>
                            <li class="px-6 py-3 bg-gray-50">
                                <div class="text-sm font-medium text-gray-900">
                                    <i class="fas fa-folder mr-2"></i><?php echo htmlspecialchars($pagePath); ?>
                                </div>
                            </li>
                        <?php endif; ?>
                        <li class="px-6 py-4">
